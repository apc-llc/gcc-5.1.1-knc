...
(define_attr "enabled" ""
  (cond [(eq_attr "isa" "x64") (symbol_ref "TARGET_64BIT")
	 ...
	 (eq_attr "isa" "sse4_noknc")
	   (symbol_ref "TARGET_SSE4_1 && !TARGET_KNC")
	 ...
	 (eq_attr "isa" "noknc") (symbol_ref "!TARGET_KNC")
	]
	(const_int 1)))
...
(define_insn "*movti_internal_knc"
  [(set (match_operand:TI 0 "register_operand"   "=!r")
	(match_operand:TI 1 "misaligned_operand" "o"))]
  "TARGET_KNC"
  "#"	
  [(set_attr "mode" "DI")])

(define_insn "*movti_internal_knc_2"
  [(set (match_operand:TI 0 "misaligned_operand" "=o")
      ...
      if (TARGET_KNC)
	{
	  if (REG_P (operands[0]) && REG_P (operands[1]))
	  ...
	  else
	    {
	      if (REG_P (operands[0]))
		{
		  /* For KNC, load 4 x 32bit elements.  */
		  gcc_assert (!misaligned_operand(operands[1], TImode));
		  return "vbroadcasti32x4\t{%M1, %g0|%g0, %M1}";
		}
	      else
		{
		  /* No store on KNC.  */
  ...
  switch (get_attr_type (insn))
      ...
      if (TARGET_KNC)
	{
	  if (get_attr_mode (insn) == MODE_TI)
	    ...
	    {
	      /* For KNC, load one 64bit element.  */
	      gcc_assert (!misaligned_operand(operands[1], DImode));
	      return "vpbroadcastq\t{%M1, %g0|%g0, %M1}";
	    }
	  else
	    {
	      /* Store is handled by secondary reload.  */
	      gcc_unreachable ();
	    }
	}
  ...
  "!(MEM_P (operands[0]) && MEM_P (operands[1]))"
{
  switch (get_attr_type (insn))
	...
	switch (which_alternative)
	  ...
	  case 7:
	      return "vmovdqa32\t{%1, %0|%0, %1}";
	  case 8:
	    /* For KNC, load one 32bit element.  */
	    gcc_assert (!misaligned_operand(operands[0], SImode));
  ...
  [(set (attr "isa")
     (cond [(eq_attr "alternative" "6,9")
	      (const_string "noknc")
	    (eq_attr "alternative" "11")
	      (const_string "sse4_noknc")
	   ]
       (const_string "*")))
   (set (attr "type")
     (cond [(eq_attr "alternative" "2")
  ...
  "!(MEM_P (operands[0]) && MEM_P (operands[1]))"
{
  switch (get_attr_type (insn))
    ...
    case TYPE_MSKMOV:
      switch (which_alternative)
        {
	case 4: return TARGET_KNC ? "kmov\t{%k1, %0|%0, %k1}" : "kmovw\t{%k1, %0|%0, %k1}";
	case 5: return TARGET_KNC ? "kmov\t{%1, %0|%0, %1}" : "kmovw\t{%1, %0|%0, %1}";
	case 6: return TARGET_KNC ? "kmov\t{%1, %k0|%k0, %1}" : "kmovw\t{%1, %k0|%k0, %1}";
  ...
  "!(MEM_P (operands[0]) && MEM_P (operands[1]))"
{
  switch (get_attr_type (insn))
	...
	case 7:
	  if (TARGET_AVX512DQ)
	    return "kmovb\t{%k1, %0|%0, %k1}";
	  else if (TARGET_KNC)
	    return "kmov\t{%k1, %0|%0, %k1}";
	  else
	...
	case 9:
	  if (TARGET_AVX512DQ)
	    return "kmovb\t{%1, %k0|%k0, %1}";
	  else if (TARGET_KNC)
	    return "kmov\t{%1, %k0|%k0, %1}";
	  else
...
(define_split
  [(set (match_operand:TF 0 "push_operand")
	(match_operand:TF 1 "register_no_elim_operand"))]
  "TARGET_KNC"
  [(set (reg:P SP_REG) (plus:P (reg:P SP_REG) (const_int -16)))
   (set (match_dup 0) (match_dup 1))]
{
...
(define_insn "*movtf_internal"
  ...
  "(TARGET_64BIT || TARGET_SSE
    || (TARGET_KNC
	&& (!MEM_P (operands[0])
	    || !misaligned_operand(operands[0], SImode))
	&& (!MEM_P (operands[1])
	    || !misaligned_operand(operands[1], SImode))))
   && !(MEM_P (operands[0]) && MEM_P (operands[1]))
   && (!can_create_pseudo_p ()
  ...
  switch (get_attr_type (insn))
      ...
      if (TARGET_KNC)
	{
	  switch (which_alternative)
	    ...
	    case 14: /* m -> v  */
	    case 18: /* m -> *x  */
	      return "vbroadcastsd\t{%M1, %g0|%g0, %M1}";
	    case 15: /* v -> m  */
	    case 19: /* *x -> m  */
	      /* For KNC we shouldn't reach here.  */
	      /* return "vstoreq\t{%1%{a%}, %M0|%M0, %M1%{a%}}"; */
	      gcc_unreachable ();
	    default:
	      break;
	    }
...
(define_insn "*movsf_internal"
  ...
  "!(MEM_P (operands[0]) && MEM_P (operands[1]))
   && (!TARGET_KNC
       ...
       || !((MEM_P (operands[0])
	     && misaligned_operand(operands[0], SFmode))
	    || (MEM_P (operands[1])
		&&  misaligned_operand(operands[1], SFmode))))
   && (!can_create_pseudo_p ()
       || (ix86_cmodel == CM_MEDIUM || ix86_cmodel == CM_LARGE)
       || GET_CODE (operands[1]) != CONST_DOUBLE
       || (optimize_function_for_size_p (cfun)
  ...
  switch (get_attr_type (insn))
    ...
    case TYPE_SSEMOV:
      if(TARGET_KNC)
	    ...
	    case 7:
	        return "vbroadcastss\t{%M1, %g0|%g0, %M1}";
	    case 8:
	      gcc_unreachable ();
	    default:
	      break;
